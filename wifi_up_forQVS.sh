...
#For QVS, to use the existing command line, which is wrong, in respect, but due to NVQM limitation, had to write a hack for this
#To fit the run even when the command given is "wifi_bluetooth_stress --wifi 5"
MODULE=bcm43241
[ `echo $0 | grep wifi | wc -c` -gt "10" ] && set - "-a" "nvtestwireless" "-p" "Sp33doflight" "-i" "wlan0"
#QVS BYPASS CODE ENDS
